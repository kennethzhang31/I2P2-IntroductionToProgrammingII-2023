...
Enemy::Enemy(std::string img, float x, float y, float radius, float speed, float hp, int money, int id) :
	Engine::Sprite(img, x, y), speed(speed), hp(hp), money(money), maxSpeed(speed), slowRemainTime(0), enemy_id(id) {
...
void Enemy::SlowDebuff(float slowFactor, float slowTime) {
